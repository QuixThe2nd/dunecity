...
inline Profile profile(int difficulty, int tech) {
    ...
    switch (difficulty) {
        case 0: return {1, 3+stage, (3+stage)*300, 180000-stage*30000, 120000, 150000, 25};
        case 1: return {1, 6+stage, (6+stage)*425, 120000-stage*15000, 60000, 180000, 15};
        case 2: return {2, 10+stage*2, (10+stage*2)*550, 75000-stage*15000, 0, 240000, 10};
        default:return {8, 16+stage*4, (16+stage*4)*700, 40000-stage*10000, 0, 300000, 5};
    }
}
struct Wave {
...
inline int requiredArmy(const Profile& p, int configuredThreshold) {
    // A small permitted sortie must not wait for a much larger legacy army goal.
    return std::min(std::max(0,configuredThreshold),p.value);
}
inline bool canLaunch(const Profile& p, const Pressure& used, uint32_t now,
                      uint32_t opening, uint32_t recoveryCycles) {
    return now >= opening && used.houses < p.houses && used.units < p.units
        && used.value < p.value
...
inline bool fits(const Profile& p, const Pressure& used, int value) {
    return value > 0 && used.units < p.units && value <= p.value-used.value;
}
