...
def package_unit(args: argparse.Namespace) -> int:
    ...
    packaged = 0
    for source_state, manifest_state in SUPPORTED_STATES.items():
        category = metadata.get("categories", {}).get(source_state, {})
        ...
        for direction in DIRECTIONS:
            ...
            animation = state.get("assets", {}).get("animation", {})
            frames_value = animation.get("frames_dir", "")
            if not frames_value:
                continue

            frames_dir = asset_root / frames_value
            frames = load_frames(frames_dir, args.frame_size)
            if not frames:
                print(f"skip {source_state}/{direction}: no enhanced PNG frames")
                continue
            ...
            packaged += 1
            print(f"packaged {source_state}/{direction}: {len(frames)} frames")
