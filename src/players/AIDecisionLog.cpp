...
uint64_t DecisionLog::write(uint32_t cycle, int house, int player,
    ...
    const auto row = Record().set("schema_version", 1).set("telemetry_version", 13).set("policy_version", "city-capacity-recovery-v69")
    ...
    if (bytes + row.size() > limit) {
        ...
        stream << Record().set("schema_version", 1).set("telemetry_version", 13).set("policy_version", "city-capacity-recovery-v69").set("session", session).set("seq", id)
