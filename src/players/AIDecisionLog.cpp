...
uint64_t DecisionLog::write(uint32_t cycle, int house, int player,
    ...
    const auto row = Record().set("schema_version", 1).set("telemetry_version", 14).set("policy_version", "funded-growth-opening-v70")
    ...
    if (bytes + row.size() > limit) {
        ...
        stream << Record().set("schema_version", 1).set("telemetry_version", 14).set("policy_version", "funded-growth-opening-v70").set("session", session).set("seq", id)
