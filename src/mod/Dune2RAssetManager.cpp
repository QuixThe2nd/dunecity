...
void Dune2RAssetManager::loadCatalog() {
    const std::filesystem::path catalogPath = std::filesystem::path(modPath) / "asset-catalog.ini";
    INIFile catalog(catalogPath.string());
    ...
    if(revision.empty() || baseURL.rfind("https://", 0) != 0 || packCount < 0 || packCount > 256) {
    ...
    for(int packIndex = 0; packIndex < packCount; ++packIndex) {
        ...
        for(int fileIndex = 0; fileIndex < fileCount; ++fileIndex) {
            ...
            try {
                file.size = std::stoull(fields[1]);
            } catch(const std::exception&) {
                THROW(std::runtime_error, "Invalid file size in Dune2R asset pack " + pack.id);
            }
            if(file.size == 0) {
                THROW(std::runtime_error, "Empty files are not allowed in Dune2R asset packs");
            }
