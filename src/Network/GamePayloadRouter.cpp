...
bool GamePayloadRouter::handle(Uint32 packetType, InputStream& stream, GamePayloadPeer& peer,
    ...
    switch(packetType) {
        ...
        case NETWORKPACKET_JOIN_ACK: {
            ...
            if(data.size()>48u*1024 || transaction==0 || (packetType==NETWORKPACKET_JOIN_SYNC && (operation<1 || operation>3))
               || (packetType==NETWORKPACKET_JOIN_ACK && (operation!=0 || !data.empty()))
